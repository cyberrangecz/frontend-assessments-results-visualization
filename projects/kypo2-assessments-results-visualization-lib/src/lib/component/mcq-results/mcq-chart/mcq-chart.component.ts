...
@Component({
  selector: 'kypo2-viz-assessments-results-mcq-chart',
  template: '<div id="chart-container" #chart></div>',
  encapsulation: ViewEncapsulation.None,
  styleUrls: ['./mcq-chart.component.css']
})
export class MCQChartComponent implements OnInit, OnDestroy {
  ...
  initializeScales() {
    this.yScale = this.d3.scaleBand()
      ...
      .domain(this.question.options.map((choice, index) => index.toString()))
      .padding(0.2);
  ...
  createYAxis() {
    ...
    yAxis.tickFormat((tickValue) => {
      // countedAnswers before
      const text: string = this.question.options[tickValue];
  ...
  highlightCorrectCircles() {
    ...
    this.question.options.forEach((option, optionIndex) => {
      if (this.question.isCorrectAnswer(optionIndex)) {
        this.question.filterAnswersByChoice(optionIndex).forEach((answer, index) => {
            this.svgElement.append('g')
              .attr('class', 'bar-highlighted')
              .append('rect')
              .attr('x', this.xScale(1) - 2 * circleRadius - padding)
              .attr('y', this.yScale(optionIndex.toString()))
  ...
  createCircles() {
    ...
    this.question.options.forEach((option, index) => {
      this.svgElement.selectAll('.player choice-order-' + index)
        .data(this.question.filterAnswersByChoice(index))
        .enter()
        .append('circle')
        .attr('class', (answer) => 'player player-' + answer.trainee.getLoginWithoutSpecialChars())
        .attr('cx', (answer, i) => this.xScale(i + 1) - circleRadius)
        .attr('cy', this.yScale(index.toString()) + this.yScale.bandwidth() / 2) // Align to center
        .attr('r', circleRadius);
    });
  }
  ...
  createSumColumn() {
    ...
    sumColumn.selectAll('.sum-value')
      .data(this.question.options)
      .enter()
      .append('text')
      .attr('class', 'sum-value')
      .attr('x', this.options.chart.width + this.options.stats.sum.marginLeft)
      .attr('y', (choice: string, index) => this.yScale(index.toString()) + this.yScale.bandwidth() / 2)
      .html((choice: string) => this.question.calculateSameAnswersCount(choice).toString());
  }
...
// TODO: Check if rewritten correctly
    percentageColumn.selectAll('.percentage-value')
      ...
      .attr('x', x)
      .attr('y', (option: string, index) => this.yScale(index.toString()) + this.yScale.bandwidth() / 2)
      .html((option: string) => (this.question.calculateMatchingAnswersPercentage(option).toFixed(1)));
  }
  ...
  createPlayerTooltip() {
    ...
    players.on('mouseover', (playersAnswer: MCQAnswer, i, nodes) => {
      const node = nodes[i];
  ...
  createOptionTooltip() {
    ...
    choiceTicks.on('mouseover', (tickOrder: number, i, nodes) => {
      const choiceTitle = this.question.options[tickOrder];
